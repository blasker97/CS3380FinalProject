<?php
	...
	class GolfModel {
		...
		public function getGames() {
			...
			$sql = "SELECT * FROM golf WHERE userID = $userIDEscaped ORDER BY $orderByEscaped $orderDirectionEscaped";
		...
		public function getGame($id) {
			...
			$sql = "SELECT * FROM golf WHERE userID = $userIDEscaped AND id = '$idEscaped'";
			if ($result = $this->mysqli->query($sql)) {
				if ($result->num_rows > 0) {
